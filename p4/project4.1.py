...
frame = "pytorch"  # 可选： "keras", "pytorch", "tensorflow" (我林哲只用pytorch)
if frame == "pytorch":
    from LSTM_pytorch import train, predict
else:
...
class Config:
    ...
    time_step = 60              # 这个参数很重要，是设置用前多少天的数据来预测，也是LSTM的time step数，请保证训练数据量大于它
    ...
    if do_train and (do_log_save_to_file or do_train_visualized):
        cur_time = time.strftime("%Y_%m_%d_%H_%M_%S-林哲", time.localtime())
...
def load_logger(config):
    ...
    if config.do_log_print_to_screen:
        ...
        formatter = logging.Formatter(datefmt='%Y/%m/%d %H:%M:%S-lz-20212211322',
